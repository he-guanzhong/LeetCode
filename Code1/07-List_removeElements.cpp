...
// 首系列元素等于目标值，则不断循环while，挪动head的起始位置。
// 直至其不存在（删干净了）或到达第一个不等于目标值的结点
// 删除后序结点，p和p->next必须同时存在。
// 注意删除时p不能挪动，因为下一个补上来的结点可能仍要删除。不删除时，p才能挪动
...
// 先排除空指针异常，然后分三种情况：
// 一、删除结点不是尾结点，将删除结点下一节点赋值，并删除，删除节点指向下一节点之后，复杂度O(n)
