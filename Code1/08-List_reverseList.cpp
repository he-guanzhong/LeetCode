...
// 优选双指针，全局保存过去、现在、未来三个结点。
// 无需对head特殊情况进行处理。最终返回更新后的pre，即为新的头结点
// 次选递归。退出条件为找到原尾节点，即返回。
// 不断向下递归head->next，标记新的头结点last。此时该层head为倒数第二结点。
...
// 双指针法。全局化过去，现在，未来三个结点。
// 只要现在结点p存在，则暂存未来tmp，反转p指向。更新过去pre和现在p的位置，最终返回pre
