...
// 交换结点涉及4个结点的同时操作，虚拟头结点设为1，2和4需要额外保存。
// 因为1可以直接指3，此时会丢失2、4的链接。
// 由于需要额外保存2、3，所以遍历的条件即为2、3存在。
// 1必存在，4可以为空结点。2存在4不存在时，也不行
...
// 另一种思路，设结点序号为0、1、2、3。0作为前置结点必存在。
// 由于要调换1、2的位置，故1、2必存在，构成while条件
