...
// 步骤一、遍历确定起始点a，去重，注意要先处理该元素，
// 如果于上一元素相等再跳过，不能预先判断该元素于下一元素相等就掉过
// 步骤二、设定b=a+1,c=nums.size()-1。三者求和，根据和大小，分别挪动b\c
// 如=0，先压入结果，再在b<c空间内去重，共同向中间收敛。
// 去重，核心在于先处理，再去重，因为组内允许相等元素出现。若先去重、再处理，相当于不允许组内相等元素
