...
// 核心思路：前中后整体去重+整体反转+正向遍历反转单词。
// 不得新增字符串，从后向前遍历老字符串，每遇到空格就加入
// 注意细节：整体去重中，每次要整体单词一并赋值，非首次赋值，还要额外补充空格。
// 反转单词中，注意考虑reverse左闭右开区间，要包括i==size
...
// 第二步，反转整个字符串。设置慢指针，遍历时发现空格或者到终止，即反转单词。
// 更新慢指针位置。注意reverse是左闭右开区间，故i<=size()
...
// 先除去所有多余空格，利用双指针，只考虑有字母的位置，
// 若慢指针起始点不为0则补空格，其余直接拷贝整个单词
...
// 使用C++特性的做法。从后向前，找到第一个非空元素后，以此为right，
// i继续前移拓展非空元素，直至第一个空元素，或首元素。结果字符串叠加之，并叠加空格
