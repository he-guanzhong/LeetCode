...
// 二叉搜索树核心性质：中序遍历后，相邻两个结点严格递增的，不得相等。
// 递归法一、可以使用vector作为中序遍历结果。
// 递归法二、中序遍历时就比较该结点值与上一结点值。
// 注意上一结点值要保存为8字节的longlong，因为测试用例中有4字节的最小值
// 迭代法，简化的中序遍历。设置当前、上一双指针，不必提前压入栈。
// 循环中，cur存在就压入，并左移。cur不存在就去栈首，处理后，cur右移
...
// 退出条件为，栈空或者当前结点为空。
// 若当前结点不空，则入栈，然后移动结点坐标。当前结点空，则取头位处理，并坐标右移
