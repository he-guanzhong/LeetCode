...
// 后序遍历，底部向上搜索并回溯。必须要搜索整个树/
// 且题目说明树中必定存在p、q，故使用临时左右结点，接受左右递归上来的结点
// 如果底部搜索到p或q，直接返回（如此处理了q是p更底层这类情况）。
// 若某结点左右递归结果结点都存在，则必为公共祖先，直接返回
// 同样，对一边返回结点、一边返回空结点的情况，返回有结点的那一方。如此即可把公共祖先结点上传。
// 后序遍历使用回溯。传入三个待比较结点。传出共同结点寻找到的结果。退出条件为，找到p、q结点，或空结点
