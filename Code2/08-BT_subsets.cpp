...
// 子集问题，要记录所有结点，而非叶子结点。可以没有退出条件，
// 因为每次传入的start为i+1，遍历到nums.size即终止
// 组合、分割问题为仅处理叶子结点。所以压入结果要在返回之前。
// 返回的条件为集合为空时，即startIndex要大于nums.size()，
// 此处可写可不写，因为不在横向遍历范围之内
// 传入参数为startIndex，因为已经选取过的元素不能重复选取，下递归时传入i+1。
// 时间复杂度n*2^n
