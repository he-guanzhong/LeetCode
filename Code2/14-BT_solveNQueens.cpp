...
// 棋盘必须全部初始化为'.'。回溯函数传入深度，即行数row。
// 退出条件为行数等于n。横向遍历列数col。如果该位置valid有效才进入下一层循环
// 有效valid函数，三个判断。1、固定列数，判断以上每一行是否有Q。
// 2、从该起点同一层for，必须同步，沿左上方向寻找Q。3、右上方向寻找Q
...
// 判断某位置是否可以放置Q：1.同列内不同行数位置未出现过Q；
// 2.左上方45度所有位置未出现过Q；3.右上方135度位置未出现过Q
