...
。如果 s[j] >=
g[i]，我们可以将这个饼干j分配给孩子i，这个孩子会得到满足。你的目标是尽可能满足越多数量的孩子，并输出这个最大数值。
示例 1:
输入: g = [1,2,3], s = [1,1]
输出: 1
解释:
...
// 注意：先排序，默认为从小到大，从大到小需要greater<int>()。固定的index最好外置于for，其即代表最终满足了的孩子数，不必专门result
// 细节：挪动固定index时，必须先做合规性检查，再使用下标取值，否则空集报错。合规性价差如做到for循环内，则必须&&
...
// 贪心算法，要先排序，优先用大饼干喂大孩子。拿着最大的饼干试孩子。故一定先固定饼干index，遍历孩子。因为最大饼干必给出
// 不能对最大的孩子试饼干。固定孩子index，遍历饼干，因为最大饼干不一定满足最大孩子的需求，
...
// 换思路，小饼干喂饱小孩子。对着最小的孩子试饼干。故固定孩子，遍历饼干。如此，index即表达了最多孩子数，无需result
