...
// 排除一个元素不用跳的情况。如不想考虑特殊情况，则跳到nums.size()-1位置为止。
// 记录当前覆盖，和下段覆盖。下段覆盖不断更新，
// 当前下标遇到当前覆盖范围，即更新当前覆盖范围，并结果加一。
// 当前覆盖范围一旦大于末尾，返回结果。
// 取巧，题目规定必可到达终点。在倒数第二位nums.size()-2处停下。
// 如此不必判断当前覆盖是否超过了最大边界。

// 最小的步数覆盖最大的范围。排除已经在终点不同跳的意外情况。
// 记录当前能到的最远距离，下一阶段能到的最远距离。当前距离内，更新下一步能到的最远距离
// 当光标移动到当前距离末端，意味着一定要再跳一步，更新此时的当前最远距离。
// 额外判断，如当前最远距离大于重点，则不必判断，直接输出
