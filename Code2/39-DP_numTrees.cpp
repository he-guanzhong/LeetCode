...
// dp[i]为i个节点的二叉搜索树数量。拆解为用头节点j切分[1,i]区间
// 所有左子树[1,j]二叉树数量*右子树[j,i]二叉树数量之和。
// 初始化dp[0]=1，空子树也算二叉搜索树，数量为1。一轮遍历从[1,n]为节点数。
// 二轮遍历为[1,i]该节点数下变换头节点，不断累积二叉树数量

// dp[i]意为到i的不同二叉搜索树数量。其可以拆解为，左子树数量先0右子树i-1个，相乘
// 然后左子树不断增多至i-1右子树不断减少至0，相乘。
