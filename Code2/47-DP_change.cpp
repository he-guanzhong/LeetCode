...
// 递推公式，dp[j]组合数为上一轮金额j组合数，和本轮减去coins[i]组合数之和。
// 组合数遍历顺序必须先物品再背包，否则为排列数。

// 硬币可以无限取，完全背包问题。但是本题求的是数量，而非最大价值。
// 故dp[j]表示背包容量，即总金额为j时的可能组合数量
// 递推公式dp[j]+=dp[j-coins[i]]，即金额为j的组合数量，
// 等于上一轮金额j的组合数量，加本轮刨去硬币coins[i]的数量
// 初始化注意，dp[0]=1，因为金额总和为0只有一种方法。
// 容量1的背包放价值1的硬币，只有一种可能。但是其余起始金额j总和均初始化为0。
