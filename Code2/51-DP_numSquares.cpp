...
// 由于求的是最小数量，所有dp初始值必须要为最大INT_MAX，才不会被min函数覆盖。
// dp[0]=0因为填满0的背包不需要完全平方数
...
// 无限取平方数，完全背包。容量为n，物品为i*i
// 先初始化为INT_MAX，由于平方数必从1开始，第一轮遍历即消除所有INT_MAX
// 后续递推公式，无需判断是否可能。
// dp[j]表示数j由完全平方数组成的最小数量，其值等于减去本轮i*i的数量加一。
// 也可以先遍历背包，再遍历物品
