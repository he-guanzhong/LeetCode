...
// dp[i]表示到第i个字母（其下标为i-1）为止的字符串能否被组成。
// 固定i，在[0,i]内使用j分割为两部分。j范围为[0,i)，因为起始允许空分割，终止下标为i-1
// s子数组长度为i-1-j+1=i-j。注意，dp[0]=true空集必可以组成
// dp[i]是否为真，取决于dp[j]是否为真，和单词[j,i]是否能找到
