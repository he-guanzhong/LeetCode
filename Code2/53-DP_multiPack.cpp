...
// 多重背包，每个物品是有限的，可以转化为01背包问题
int multiPack(vector<int> &weight, vector<int> &value, vector<int> &nums,
              int bagWeight) {
...
// 时间复杂度，m*n*k。
int multiPack1(vector<int> &weight, vector<int> &value, vector<int> &nums,
               int bagWeight) {
  for (int i = 0; i < nums.size(); i++) {
...
int multiPack2(vector<int> &weight, vector<int> &value, vector<int> &nums,
               int bagWeight) {
  vector<int> dp(bagWeight + 1, 0);
  for (int i = 0; i < weight.size(); i++) {
    for (int j = bagWeight; j >= weight[i]; j--) {
