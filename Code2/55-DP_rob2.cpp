...
int maxVal(vector<int>& nums, int l, int r) {
  ...
  int n = r - l + 1;
  vector<int> dp(n, 0);
  dp[0] = nums[l], dp[1] = max(nums[l], nums[l + 1]);
  for (int i = 2; i < n; i++)
...
int rob(vector<int>& nums) {
  if (nums.size() == 1)
    return nums[0];
  return max(maxVal(nums, 0, nums.size() - 2),
             maxVal(nums, 1, nums.size() - 1));
}

// 拆分为，不考虑首元素，和不考虑尾元素，二者情况的最大值。
// 传参可以使用起始下标，先排除0和1特殊情况影响，使用左闭右开，或左开右闭均可
