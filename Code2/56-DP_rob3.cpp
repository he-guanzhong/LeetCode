...
除了 root之外，每栋房子有且只有一个“父“房子与之相连。一番侦察之后，
...
// 退出条件为，空结点，无论偷不偷都是0。叶子结点，不偷必为0，偷就是该点值大小。
// 后序遍历，保存左右分支值。若偷该结点，则左右结点必不能偷，取其[0]值。
// 若不偷该结点，则左右均可偷可不偷取最大值，该结点值为左右最大值之和
// 以不偷和偷顺序，返回数组。
// 时间复杂度n，因为每个结点只计算了一遍，空间复杂度logn，算上了递归系统栈空间
...
// 暴力递归搜索。一定是后序遍历，因为要利用返回值，当作该点最大值的结果。
// 退出条件是空结点，返回0。叶子结点，返回结点值。
// 情况可以分为，1、抢该节点，则该节点值需要加上可能的孙结点返回值。
// 2、不抢该结点，该节点值应为左右孩子返回值之和
...
// 使用记忆化递归，如果计算过的结果，就不再重复计算。
// 时间复杂度n因为每个节点都算了一次，空间复杂度logn，算上系统递归栈所用空间
