...
// 动态规划，dp[i][0]表示第i天持有股票钱数，dp[i][1]表示第i天不持有股票钱。
// 初始化dp[0][0]为首天价格负数。dp[0][1]为0。
// 递推公式。第i天持有股票，分i-1天就不持有，或i天买入两种情况。
// 第i天不持股，分i-1天就不持股，或i天卖出（第i-1天持有+i天股票价值）两种情况。分别取其最大值
...
// 动态规划，当天持有，等于前一天就持有，或者今天第一次直接买入，二者最大值。
// 当天不持有，等于前一天就不持有，或者今天卖出，二者最大值。
// 贪心。最大收益就是从左往右遍历，找到当前最小值。
// 同时计算该起点继续遍历找到的，最大值差值。时间复杂度n，空间复杂度1
