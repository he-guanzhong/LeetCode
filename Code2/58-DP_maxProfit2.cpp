...
// 优选贪心，相邻两天相减，收集所有正利润。
// 动态规划。该天持有股票最大利润dp[0]等于前一不持有，或前一天持有当天卖出的最大值。dp[1]亦然。
// 滚动数组，无需临时保存dp[0]，因为允许当天买卖，对结果无影响。

// 动态规划。dp[0][0]代表第0天持有股票，则初始化为负数。
// dp[0][1]代表第0天不持股。空间复杂度O(n)，空间复杂度O(n)
// 注意此题与前提区别，股票可以买卖多次，
// 故第i天持股，dp[i][0]考虑当天买入股票-prices[i]时，前一天不持股dp[i-1][1]可能有利润
...
// 贪心。第三天利润等于第一天和第二天利润和。
// 故只要是每隔一天是正利润，就计入结果。时间复杂度O(n)
