...
示例 2：
  ...
  解释：在第 1 天（股票价格= 1）的时候买入，
        ...
        注意你不能在第 1 天和第 2天接连购买股票，之后再将它们卖出。
...
// dp[i][0~5]记录第i天，第一次买入1，第一次卖出2，第二次买入3，第二次卖出4，四种状态的最大利润额。
// 初始化时，第一次买入和第二次买入，均为-prices[0]
// 递推公式，第一次持有，注意值肯定是-prices[i]。
// 第三次持有为本来就持有，或第一次卖出减去当前股票价格的最大值
// dp[i][3]=max(dp[i-1][3],dp[i-1][2]-prices[i])
// 第二次卖出，等于本来就不持有，或之前第二次持有，加上本次股票价格
...
// 0/1/2/3分别对应第一次持有，不持有，持有，不持有状态。
// 注意允许当天买卖，故只要持有，均初始化为-prices[0]。
// 同理，由于当天允许买卖，故dp[1]第一次不持有，不必上一周期考虑dp[0]，
// 本周期dp[0]也可，因为当天卖买结果为0，无影响
