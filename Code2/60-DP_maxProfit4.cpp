...
// dp[i][2*j-1]表示第i天，第j次持有股票，所得到的利润。
// dp[i][2*j]表示第i天，第j次不持股票，得到利润，dp总和为2k+1
...
// 代码统一性，dp[i]表示持有与否的的最大金额。
// dp[1,2k]代表第k次持有、不持有的最大金额，不要使用dp[0]。
// 对所有代表持有的dp[i]，均初始化为-prices[0]
// 从第二件物品i=1开始，利用dp[j]对k次持有、不持有情况遍历，每次+=2。
// 最终返回第k次不持有的最大金额。
