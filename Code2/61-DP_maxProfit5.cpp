...
// 标准解法，dp[i][j]表示第i天第j状态的最大利润。
// dp[i][0]为持股，dp[i][1]为不持股不冷冻，dp[i][2]为当天卖出，dp[i][3]为冷冻期
// 初始化，dp[0][0]当天持股，利润为-prices[0]。其余三种情况都不持股，均为0
// 递推公式。持股，dp[i][0]为昨天就持股，或昨天不持股不冷冻+今日股价的成本，或者冷冻+今日股价的成本。三者最大值
// 不持股且不冷冻，dp[i][1]为昨天不冷冻，或昨天是冷冻，二者最大值
// 当天卖出，dp[i][2]为昨天持股加当日股价利润。今天冷冻期，dp[i][3]为昨日刚卖过股票
// 最终输出最大利润为，不是持股的三种情况，最大值
// 注意，四种定义可以变换，例如dp[j]表示，1为持有，2为卖出，3为卖出冷冻，4为不持有不冷冻
// 此时持有，分三种情况，昨天就持有，昨天不冷冻今天买入，昨天冷冻今天买入。
...
// 自己解法，将前两天的情况独立出来。从第三天开始计算，
// 第i天持股值dp[i][1]一定是上一天也持股dp[i-1][1]和前天不持股，今天买入的最大值
