...
代表了交易股票的手续费用。你可以无限次地完成交易，但是你每笔交易都需要付手续费。
...
// 贪心算法或动态规划均可。出时加收手续费。返回时要返回持有与否二者最大值，因为卖出股票可能负收益
// dp[i][0]表示第i天持股现有现金，dp[i][1]表示第i天不持股现有现金。
// 初始化dp[0][0]一开始持股，必为-prices[0]
// 递推公式，相比传统买卖股票，假定卖出股票时要计算手续费，则不持股
// dp[i][1]=max(dp[i-1][1],dp[i-1][0]+prices[i]-fee)
