...
// 递推公式，dp[i]元素为，从0~i-1中挑出的j元素
// 只要其结尾nums[j]<nums[i]即可构成子序列，子序列数+1，则dp[i]=max(dp[i],dp[j]+1)
...
int main() {
  ...
  cout << lengthOfLIS(nums1) << " " << lengthOfLIS(nums2) << " "
       << lengthOfLIS(nums3) << " " << lengthOfLIS(nums4) << endl;
  cout << lengthOfLIS1(nums1) << " " << lengthOfLIS1(nums2) << " "
       << lengthOfLIS1(nums3) << " " << lengthOfLIS1(nums4) << endl;
  return 0;
}