...
// dp[i][j]表示以t[j-1]结尾子序列，出现在s[i-1]结尾子序列个数（不是长度）。
// 联想bagg和bag二维矩阵例子，看相等的末尾元素，取或不取两种情况
...
// dp[i][j]表示以s[i-1]和t[j-1]为结尾的子序列个数（注意不是长度）。
// 由于在s中寻找子集t，则dp[0][j]相当于在空集中找字母，必不可能，为0；
// dp[i][0]相当于在s中寻找空集，必可能，为1。空集中找空集，可能，dp[0][0]=1
// 递推公式，当s[i-1]和t[j-1]元素相等时，将s和t都截取一位，
// 取dp[i-1][j-1]的子集个数；或只截s一位，t不截。取dp[i-1][j]子集个数。
