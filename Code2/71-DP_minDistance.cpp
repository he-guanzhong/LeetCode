...
// 次选，动态规划dp[i][j]表示以s[i-1]和t[j-1]末尾的元素，要想等需删除次数。
// 初始化dp[i][0]=dp[0][j]=i，即删到空需将所有字母均删除
// 末尾元素相等，无需删除dp[i][j]=dp[i-1][j-1]。
// 末尾元素不等，取删除s末尾和t末尾的最小值，不必考虑左上角，因为已经考虑过了

// dp[i][j]表示以word1[i-1]和word2[j-1]为结尾的字符串，若想相等，最少删除字符数。
// 初始化dp[i][0]表示，i长度的word1，想删除到空，须删除的字符数，故dp[i][0]=i，同理dp[0][j]=j
// 递推公式，如果两子串末字母相等，则不必增加删除字符数，dp[i][j]=dp[i-1][j-1]。
// 若末字母不等，则必须在二子串之中删除一个字符,取最小。dp[i][j]=min(dp[i-1][j]+1,dp[i][j-1]+1)
