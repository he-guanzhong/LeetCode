...
// 末尾元素相等，不增次数。
// 末尾元素不相等，要看删除s，删除t（新增s），替换s，三种情况最小值，再多加一步。最终返回矩阵右下角

// dp[i][j]表示以word1[i-1]和word2[j-1]为结尾的两子串，最小剪辑距离。
// 故dp[i][0]表示i长度子串要缩减为空，所需操作为i,dp[i][0]=i。同理dp[0][j]=j
// 递推公式，如果两子串末尾相等，则无需剪辑dp[i][j]=dp[i-1][j-1]。
// 如两子串末尾不相等，则word1必剪辑次数+1，可能为删除dp[i-1][j]，可能为新增dp[i][j-1]，可能为替换dp[i-1][j-1]
