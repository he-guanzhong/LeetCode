...
// dp[i][j]表示以下标[i,j]的字符串是否是回文串，只有0和1两种状态。
// 初始化二维矩阵，左上到右下斜线为1，因为一个字母必回文
// 从左下到右上遍历，j以i起始即可，不必专门初始化
// s[i]两元素s[j]相等，如果相差为0或1或2，则必回文，如果相差较大，需看[i+1,j-1]是否回文
// 统计回文出现的次数，返回。空间复杂度n^2太高，考虑双指针法。空间复杂度O(n)=1
// 对每一个元素，沿左右拓展，遇到边界或不等就停止，返回检验到的回文数。
// 对相邻元素相等例子，如abbc，则需传入[i,i+1]双起点
