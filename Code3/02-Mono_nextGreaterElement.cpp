...
// 如果新元素大于栈顶元素，则必须将栈顶元素弹出，直至栈顶元素为小于栈内元素。弹出过程中，如果发现即将弹出的是nums1中的元素(使用count判断)
...
// 单调栈方法二，umap记录nums2每一个弹出nums2[i]，和下一个大于其的值。遍历nums1，只要在该元素曾弹出过（umap的key相等），将value赋值给结果
