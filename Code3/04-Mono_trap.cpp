...
// 单调栈求行雨水。从栈底到栈头从大到小排列。遇下一个更大的元素，根据左柱、底部、右柱三个信息计算。
// top为雨水底，弹出后若栈仍不空，才有左柱可以计算

// 单调栈，从栈底到栈顶，按照从大到小排序，可以求下一个比本元素更大的位置。
// stack中记录下标即可。若新元素比栈顶元素小，直接压入。
// 新元素等于栈顶元素，则将老元素弹出，再压入新元素，如此是为了保证接雨水的左柱实时更新
// 新元素大于栈顶元素，则记录此时st.top()为坑底mid，弹出后，新的st.top()即为左柱，i为右柱，
// 则接雨水的高为min(height[i],height[st.top()])-height[mid]
...
// 暴力解法。双指针，求每一列的雨水高度，总和即为所有雨水和。
// 第一列和最后一列不能接雨水，对于每一列，分别记录其左侧、最右侧最大柱高
// 左右最大柱高的最低值，就是雨水面，本列高度就是雨水底，求差。
// 如果差为正，即可接雨水，累计求和。时间复杂度O(n^2)，超时。不可使用。
...
// 优化的双指针，使用额外数组maxLeft和maxRight记录每个位置最左侧和最右侧柱子的最大值。
// 注意左侧右侧，第一个起始元素要特殊初始化
// 时间复杂度O(n)
