...
void dfs(vector<vector<int>>& graph,
  ...
  for (int i = 0; i < graph[index].size(); i++) {
    ...
    dfs(graph, graph[index][i], path, ans);
    path.pop_back();
  }
}
...
// 先压入0节点，传参为刚刚压入的节点编号，返回条件是其编号等于最后一个下标。
// for循环内遍历当前下标可以到达的节点。压入、判断、弹出
// 假定当前已遍历节点为x，退出条件即为遍历到的节点，已到达图的最末尾节点graph.size()-1，
// 则将path压入result返回，因为有向无环图，必可以到终点
// 深度优先搜索，对于该节点x，将其链接的下一层节点graph[x][i]
// 先压入path，然后进入递归，回溯时要pop弹出本层曾经压入的节点。
