...
// 注意u->v中，u是父节点，v为子节点。因此v的入度即为其父结点数。
// 题目说明，每个结点只有一个父节点。故所有结点的入度均为1，除了根节点入度为0
// 异常分三种情况。2入度的要看删除那条边成树。入度为0，必成环要删除成环边。
// 故首先统计入度。倒序遍历所有边，记录其入度为2结，在edges中的下标，入vec
// 若vec内不为空，其大小size必为2。则用一个函数看删除二者（遇到其下标直接跳过即可）哪个可以成树，返回该下标对应边。
// 若vec为空，则用一个函数，直接删除成环边。

// 对于有向图，想改树，必先遍历所有边的第二个数值，统计每个结点的入度。
// 遍历所有边，找到入度为2的结点。记录边下标。
// 此过程必反向遍历，因为最后要删除的边，如此可在vec的头两位记录与之相连边号
