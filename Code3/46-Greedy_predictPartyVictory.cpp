...
示例 2：
  ...
  解释：
    ...
    阵营的参议员可以使用第一项权利禁止第二个参议员的权利。 这一轮中，第二个来自
      Dire 阵营的参议员会将被跳过，因为他的权利被禁止了。 这一轮中，第三个来自
      Dire 阵营的参议员可以使用他的第一项权利禁止第一个参议员的权利。
...
// 贪心策略。当前位置尽量消灭后方的对方投票者。
// 模拟操作，R/D表征此轮R和D是否存在，直至其中任意一方不存在，即可宣布结果。
// flg外置，表征从起始时间到当前时刻，该位置上，R和D的相对大小关系。
// 为正，标识R>D，flg为负，标识R<D
