...
int mergeSort(vector<int>& record, int l, int r, vector<int>& tmp) {
  if (l >= r)
    return 0;
  int m = (l + r) / 2;
  int ans = mergeSort(record, l, m, tmp) + mergeSort(record, m + 1, r, tmp);
  int i = l, j = m + 1;
  for (int k = l; k <= r; k++)
    tmp[k] = record[k];
...
int reversePairs(vector<int>& record) {
  ...
  return mergeSort(record, 0, record.size() - 1, tmp);
}

// 不用使用普通方法，否则时间超限。设置分治排序函数，退出条件是左边界大于等于右边界，即只有一个元素。额外使用tmp记录原始record信息
// 对原始record改记录排序后信息，使用l和r的中间点m，切割数组。两瓣起点分别为i,j。以k，先使用tmp记录record原始数据。在左右两端[i,j]起始。
// 若i或j任意到达改半截数组的右边界，则对直接对另一赋值。若左半i小于等于右半j，不构成逆序对，直接将左半填入，
// 若左半i大于右半j，则填右半信息，并更新逆序数对。左侧总计m+1个元素，大于等于下标为i的m+1-i个
...
int main() {
  ...
  cout << reversePairs(record) << endl;
  record = {9, 7, 5, 4, 6};
  cout << reversePairs1(record) << endl;
