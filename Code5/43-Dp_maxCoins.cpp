...
nums中。 现在要求你戳破所有的气球。戳破第 i 个气球，你可以获得 nums[i - 1] *
...
// 三层遍历，从val[0,n+1]右侧开始遍历左边界i，允许范围[0,n-1]。终点为右边界j，允许范围[i+2,n+1]。
// 对[i+1,j-1]范围内k，每次取值。计算其乘积总和,并叠加以k划分(i,j)的结果，保存最大值，作为最终的dp[i][j]
