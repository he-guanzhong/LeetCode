...
// 前缀树，不仅保存节点，还要保存该节点的数值和，末尾isEnd本题无意义。另外，额外哈希表记录该字符串是否出现过，及其上一次保存值
// insert()新字符串要插入，先看之前是否存在过，如是，则取出该字符串原始值，作为被减数pre，并更新哈希表。
// 以临时变量delta，记录该字符串影响节点值是增加还是减少多少。新结点值是cur，则delta计算方法是cur-pre
