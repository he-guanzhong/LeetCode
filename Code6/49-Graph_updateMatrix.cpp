...
vector<vector<int>> updateMatrix(vector<vector<int>>& mat) {
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      if (mat[i][j] == 0)
        continue;
      int x = i == 0 ? INT_MAX / 2 : ans[i - 1][j];
      int y = j == 0 ? INT_MAX / 2 : ans[i][j - 1];
      ans[i][j] = min(x, y) + 1;
    }
  }
  for (int i = m - 1; i >= 0; i--) {
    for (int j = n - 1; j >= 0; j--) {
      if (mat[i][j] == 0)
        continue;
      int x = i == m - 1 ? INT_MAX / 2 : ans[i + 1][j];
      int y = j == n - 1 ? INT_MAX / 2 : ans[i][j + 1];
      ans[i][j] = min(ans[i][j], min(x, y) + 1);
    }
  }
  return ans;
}
